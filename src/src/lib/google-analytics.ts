...
function isSensitivePaymentPath(pathname: string): boolean {
  return (
    pathname === '/checkout' ||
...
export function sanitizeAdminPath(pathname: string): string {
  const withoutIdentifiers = pathname
    .replace(/\/profiles\/[^/]+\/chats\/[^/]+/g, '/profiles/:profile_id/chats/:chat_id')
    .replace(/\/profiles\/[^/]+/g, '/profiles/:profile_id')
    ...
    .replace(/\/pdf\/invoices\/[^/]+/g, '/pdf/invoices/:invoice_id')
    .replace(/\/\d+(?=\/|$)/g, '/:id');

  return withoutIdentifiers.replace(
...
export function trackPageView(pathname: string): void {
  if (
    ...
    isSensitivePaymentPath(pathname)
  ) {
    return;
  }
