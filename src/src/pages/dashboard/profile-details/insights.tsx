...
export function Page(): React.JSX.Element {
  ...
  const productsAvailable =
    productsAvailability?.profileId === profileId ? productsAvailability.value.available : false;
          ...
          <CardContent>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ alignItems: { sm: 'end' } }}>
              <TextField
                ...
                fullWidth
                label={t('dashboard.profiles.detail.insights.filters.from')}
                onChange={(event) => {
              ...
              <TextField
                InputLabelProps={{ shrink: true }}
                fullWidth
                label={t('dashboard.profiles.detail.insights.filters.to')}
                onChange={(event) => {
          ...
          <Alert color="info">
            {t('dashboard.profiles.detail.insights.trackingSince', {
              date: new Intl.DateTimeFormat(language, { dateStyle: 'medium' }).format(
                new Date(data.tracking_started_at)
              ),
            })}
          </Alert>
        ) : null}
...
function LineTrend({ data, t }: { data: ProfileInsights; t: TFunction }): React.JSX.Element {
  return (
    <Card>
      <CardHeader
        subheader={t('dashboard.profiles.detail.insights.charts.trendHelp')}
        title={t('dashboard.profiles.detail.insights.charts.activity')}
      />
      <CardContent>
        <ResponsiveContainer height={320} width="100%">
          <LineChart data={data.series}>
            ...
            <Line
              dataKey="new_chats"
              name={t('dashboard.profiles.detail.insights.metrics.new_chats')}
              stroke={chartColors[0]}
              strokeWidth={2}
            />
            <Line
              dataKey="total_messages"
              name={t('dashboard.profiles.detail.insights.metrics.total_messages')}
              stroke={chartColors[1]}
              strokeWidth={2}
            />
            <Line
              dataKey="unique_visitors"
              name={t('dashboard.profiles.detail.insights.metrics.unique_visitors')}
...
function ProviderFunnel({ providers, t }: { providers: ProfileInsightsProvider[]; t: TFunction }): React.JSX.Element {
  return (
    <Card sx={{ flex: '1 1 0' }}>
      <CardHeader
        subheader={t('dashboard.profiles.detail.insights.providers.help')}
        title={t('dashboard.profiles.detail.insights.providers.title')}
      />
      <CardContent>
        <Typography color="text.secondary" sx={{ display: { xs: 'block', sm: 'none' }, mb: 1 }} variant="caption">
            ...
            <TableBody>
              {providers.map((provider) => (
                <TableRow key={provider.provider}>
                  ...
                  <TableCell align="right">{provider.shown}</TableCell>
                  <TableCell align="right">{provider.opened}</TableCell>
    ...
    <Card sx={{ flex: '1 1 0' }}>
      ...
      <CardContent>
        {visible.length ? (
              ...
              <Stack key={category.key} spacing={0.75}>
                <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
                  ...
                  <Typography variant="body2">
                    {category.count} · {category.percent.toFixed(1)}%
                  </Typography>
                </Stack>
          ...
          <Typography color="text.secondary" variant="body2">
            {t('dashboard.profiles.detail.insights.categories.empty')}
          </Typography>
        )}
      </CardContent>
    </Card>
  ...
  return (
    <Stack spacing={3}>
      ...
      >
        ...
        <MetricCard
          label={t('dashboard.profiles.detail.insights.chats.averageMessages')}
          value={summary.average_messages_per_chat.toFixed(1)}
        />
        <MetricCard
          label={t('dashboard.profiles.detail.insights.chats.averageDuration')}
          value={t('dashboard.profiles.detail.insights.chats.minutes', {
            value: summary.average_duration_minutes.toFixed(1),
          })}
        />
        <MetricCard
          label={t('dashboard.profiles.detail.insights.chats.averageConfidence')}
        ...
        <MetricCard
          label={t('dashboard.profiles.detail.insights.chats.singleMessage')}
          value={summary.single_message_chats}
        />
        <MetricCard
          label={t('dashboard.profiles.detail.insights.coverage.classified')}
          value={`${data.analysis_coverage.classified}/${data.analysis_coverage.total_chats}`}
        />
      </Box>
      ...
      <Card>
        <CardHeader
          subheader={t('dashboard.profiles.detail.insights.chats.goalTrendHelp')}
          title={t('dashboard.profiles.detail.insights.chats.goalTrend')}
        />
        <CardContent>
          {trendKeys.length ? (
            <ResponsiveContainer height={320} width="100%">
      ...
      <Card>
        <CardHeader
          subheader={t('dashboard.profiles.detail.insights.chats.actionsHelp')}
          title={t('dashboard.profiles.detail.insights.chats.actions')}
        />
        <CardContent sx={{ p: 0 }}>
          <Typography
            color="text.secondary"
            sx={{ display: { xs: 'block', sm: 'none' }, px: 2, pt: 2 }}
            variant="caption"
          >
            {t('dashboard.profiles.detail.mobileTableHint')}
          </Typography>
          <Box sx={{ overflowX: 'auto' }}>
            <Table size="small">
              ...
              <TableBody>
                {data.goal_actions
                  .filter((row) => row.chats > 0)
                  .map((row) => (
                    <TableRow key={row.key}>
                      <TableCell>{goalLabel(t, row.key)}</TableCell>
                      <TableCell align="right">{row.chats}</TableCell>
                      <TableCell align="right">{row.product_click_chats}</TableCell>
                      <TableCell align="right">{row.product_click_rate.toFixed(1)}%</TableCell>
                      <TableCell align="right">{row.whatsapp_click_chats}</TableCell>
                      <TableCell align="right">{row.social_click_chats}</TableCell>
...
function Coverage({
  ...
  coverage: ProfileChatInsights['analysis_coverage'];
  t: TFunction;
}): React.JSX.Element {
  ...
  return (
    <Card sx={{ flex: '1 1 0' }}>
      ...
      <CardContent>
        <Stack spacing={2}>
          <Typography variant="h3">{percent.toFixed(1)}%</Typography>
          <LinearProgress value={percent} variant="determinate" />
          <Typography color="text.secondary" variant="body2">
            {t('dashboard.profiles.detail.insights.coverage.summary', {
              classified: coverage.classified,
              total: coverage.total_chats,
            })}
          </Typography>
          <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>
            <Chip
              label={`${t('dashboard.profiles.detail.insights.coverage.pending')}: ${coverage.pending}`}
              size="small"
            />
            <Chip
              color="warning"
              label={`${t('dashboard.profiles.detail.insights.coverage.review')}: ${coverage.needs_review}`}
              size="small"
            />
            <Chip
              color="error"
              label={`${t('dashboard.profiles.detail.insights.coverage.failed')}: ${coverage.failed}`}
              size="small"
            />
            <Chip
              label={`${t('dashboard.profiles.detail.insights.coverage.unclassified')}: ${coverage.unclassified}`}
              size="small"
            />
          </Stack>
        </Stack>
...
function ProductsReport({
  ...
  return (
    <Stack spacing={3}>
      {data.available.mode === 'historical_only' ? (
        <Alert color="info">{t('dashboard.profiles.detail.insights.products.historicalOnly')}</Alert>
      ) : null}
      <Box
        sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', lg: 'repeat(5, 1fr)' } }}
      >
        ...
        <MetricCard
          label={t('dashboard.profiles.detail.insights.products.ctr')}
          value={`${data.summary.ctr.toFixed(1)}%`}
        />
        <MetricCard
          label={t('dashboard.profiles.detail.insights.products.uniqueVisitors')}
          value={data.summary.unique_click_visitors}
        />
      </Box>

      <Card>
        <CardHeader
          subheader={t('dashboard.profiles.detail.insights.products.trendHelp')}
          title={t('dashboard.profiles.detail.insights.products.trend')}
        />
        <CardContent>
          <ResponsiveContainer height={300} width="100%">
            <LineChart data={data.series}>
              ...
              <Line
                dataKey="shown"
                name={t('dashboard.profiles.detail.insights.products.shown')}
                stroke={chartColors[0]}
                strokeWidth={2}
              />
              <Line
                dataKey="clicks"
                name={t('dashboard.profiles.detail.insights.products.clicks')}
                stroke={chartColors[1]}
                strokeWidth={2}
              />
            </LineChart>
      ...
      <Card>
        <CardHeader
          subheader={t('dashboard.profiles.detail.insights.products.tableHelp')}
          title={t('dashboard.profiles.detail.insights.products.performance')}
        />
        <CardContent sx={{ p: 0 }}>
          <Typography
            color="text.secondary"
            ...
            <Table size="small">
              ...
              <TableBody>
                {data.products.map((product) => (
                  <ProductRow key={product.key} language={language} product={product} t={t} />
                ))}
              </TableBody>
            </Table>
          </Box>
    ...
    <TableRow>
        ...
        <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center', minWidth: 220 }}>
          {product.image_url ? (
            <Box
              ...
              sx={{ borderRadius: 1, height: 44, objectFit: 'cover', width: 44 }}
            />
          ) : null}
      ...
      <TableCell>
        <Chip
          ...
          label={t(`dashboard.profiles.detail.insights.products.statuses.${product.status}`)}
          size="small"
        />
      </TableCell>
      <TableCell align="right">{product.shown.toLocaleString(language)}</TableCell>
      <TableCell align="right">{product.clicks.toLocaleString(language)}</TableCell>
      <TableCell align="right">{product.ctr.toFixed(1)}%</TableCell>
      <TableCell align="right">{product.unique_click_visitors}</TableCell>
      <TableCell align="right">{product.image_clicks}</TableCell>
      <TableCell align="right">{product.button_clicks}</TableCell>
      <TableCell>
        {product.destination_type
          ? t(`dashboard.profiles.detail.insights.products.destinations.${product.destination_type}`)
          : '—'}
      </TableCell>
      <TableCell>
        {product.goals.length
          ? product.goals.map((goal) => `${goalLabel(t, goal.key)} (${goal.chats})`).join(', ')
          : '—'}
      </TableCell>
    </TableRow>
  );
