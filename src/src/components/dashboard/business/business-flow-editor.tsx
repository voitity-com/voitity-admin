...
export function BusinessFlowEditor({ graph, onChange }: { graph: BusinessFlowGraph; onChange: (graph: BusinessFlowGraph) => void }): React.JSX.Element {
  ...
  const addNode = (type: BusinessNodeType): void => {
    ...
    const config = type === 'instruction' ? { finish_chat: false, message: '', messages: { en: '', es: '' }, wait_for_input: true } : type === 'decision' ? { branches: ['yes', 'no'], mode: 'knowledge_yes_no', question: '', questions: { en: '', es: '' }, use_business_description: true, use_sources: true } : { action: 'extract_fields', required_fields: ['full_name', 'email', 'phone', 'whatsapp', 'project_summary'] };
