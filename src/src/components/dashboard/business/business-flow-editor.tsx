...
export function BusinessFlowEditor({ graph, onChange }: { graph: BusinessFlowGraph; onChange: (graph: BusinessFlowGraph) => void }): React.JSX.Element {
  ...
  const addNode = (type: BusinessNodeType): void => {
    ...
    const config = type === 'instruction' ? { message: '', messages: { en: '', es: '' }, wait_for_input: true } : type === 'decision' ? { branches: ['yes', 'no'], mode: 'technology_interest' } : { action: 'extract_fields', required_fields: ['full_name', 'email', 'phone', 'whatsapp', 'project_summary'] };
