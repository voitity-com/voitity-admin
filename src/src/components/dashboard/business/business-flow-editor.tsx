...
import type { BusinessFlowEdge, BusinessFlowGraph, BusinessFlowNode, BusinessNodeType } from '@/lib/business/api-client';
import { findFlowNodeAtPoint, FLOW_NODE_HEIGHT, FLOW_NODE_WIDTH, getCanvasBounds, getEdgePath, getEdgePathToPoint, getEdgeTargetPoint } from '@/lib/business/flow-utils';
...
interface EdgeTargetDragState {
  edgeKey: string;
  hoverTargetKey: null | string;
  pointerId: number;
  x: number;
  y: number;
}
...
export function BusinessFlowEditor({ graph, onChange }: { graph: BusinessFlowGraph; onChange: (graph: BusinessFlowGraph) => void }): React.JSX.Element {
  ...
  const [selectedEdgeKey, setSelectedEdgeKey] = React.useState<null | string>(null);
  const [edgeTargetDrag, setEdgeTargetDrag] = React.useState<EdgeTargetDragState | null>(null);
  ...
  const changeInstructionFinish = (key: string, finishChat: boolean): void => {
    onChange({
      edges: finishChat ? graph.edges.filter((edge) => edge.source !== key) : graph.edges,
      nodes: graph.nodes.map((node) => node.key === key
        ? { ...node, config: { ...node.config, finish_chat: finishChat, ...(finishChat ? { wait_for_input: false } : {}) } }
        : node),
    });
    if (finishChat) {
      setBranch('');
  ...
  const addNode = (type: BusinessNodeType): void => {
    ...
    const config = type === 'instruction' ? { ai_instruction: '', ai_message_enabled: false, finish_chat: false, message: '', messages: { en: '', es: '' }, use_business_description: true, use_sources: true, wait_for_input: true } : type === 'decision' ? { branches: ['yes', 'no'], mode: 'knowledge_yes_no', question: '', questions: { en: '', es: '' }, use_business_description: true, use_sources: true } : { action: 'extract_fields', required_fields: ['full_name', 'email', 'phone', 'whatsapp', 'project_summary'] };
  ...
  const beginCanvasPan = (event: React.PointerEvent<HTMLDivElement>): void => {
    if (event.button !== 0 || (event.target as Element).closest('[data-flow-node],[data-flow-edge-control]')) return;
    const viewport = viewportRef.current;
    if (!viewport) return;
  ...
  const beginEdgeTargetDrag = (event: React.PointerEvent<SVGCircleElement>, edge: BusinessFlowEdge): void => {
    const target = graph.nodes.find((node) => node.key === edge.target);
    if (!target || event.button !== 0) return;
    event.preventDefault();
    event.stopPropagation();
    event.currentTarget.setPointerCapture(event.pointerId);
    const point = getEdgeTargetPoint(target, bounds.offsetX, bounds.offsetY);
    setSelectedEdgeKey(edge.key);
    setEdgeTargetDrag({ edgeKey: edge.key, hoverTargetKey: edge.target, pointerId: event.pointerId, ...point });
  };
  const moveEdgeTarget = (event: React.PointerEvent<SVGCircleElement>): void => {
    if (!edgeTargetDrag || edgeTargetDrag.pointerId !== event.pointerId) return;
    event.preventDefault();
    event.stopPropagation();
    const edge = graph.edges.find((item) => item.key === edgeTargetDrag.edgeKey);
    if (!edge) return;
    const point = canvasPoint(event.clientX, event.clientY);
    const hoverTarget = findFlowNodeAtPoint(graph.nodes, point, bounds.offsetX, bounds.offsetY, edge.source);
    setEdgeTargetDrag({ ...edgeTargetDrag, ...point, hoverTargetKey: hoverTarget?.key ?? null });
  };
  const endEdgeTargetDrag = (event: React.PointerEvent<SVGCircleElement>): void => {
    if (!edgeTargetDrag || edgeTargetDrag.pointerId !== event.pointerId) return;
    event.preventDefault();
    event.stopPropagation();
    const edge = graph.edges.find((item) => item.key === edgeTargetDrag.edgeKey);
    const point = canvasPoint(event.clientX, event.clientY);
    const nextTarget = edge
      ? findFlowNodeAtPoint(graph.nodes, point, bounds.offsetX, bounds.offsetY, edge.source)
      : undefined;
    if (edge && nextTarget) {
      onChange({
        ...graph,
        edges: graph.edges.map((item) => item.key === edge.key ? { ...item, target: nextTarget.key } : item),
      });
    }
    if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
    setEdgeTargetDrag(null);
  };
  const cancelEdgeTargetDrag = (event: React.PointerEvent<SVGCircleElement>): void => {
    if (!edgeTargetDrag || edgeTargetDrag.pointerId !== event.pointerId) return;
    event.preventDefault();
    event.stopPropagation();
    setEdgeTargetDrag(null);
      ...
      <Stack direction={{ lg: 'row', xs: 'column' }} spacing={2}>
        ...
        >
          <Box sx={{ backgroundImage: 'radial-gradient(circle, rgba(120,120,140,.24) 1px, transparent 1px)', backgroundSize: '24px 24px', height: bounds.height, position: 'relative', width: bounds.width }}>
            <Box component="svg" sx={{ height: '100%', left: 0, overflow: 'visible', pointerEvents: 'none', position: 'absolute', top: 0, width: '100%', zIndex: 2 }}>
              <defs>
                <marker id="business-arrow" markerHeight="8" markerWidth="8" orient="auto" refX="7" refY="4"><path d="M0,0 L8,4 L0,8 z" fill="#64748B" /></marker>
                <marker id="business-arrow-selected" markerHeight="8" markerWidth="8" orient="auto" refX="7" refY="4"><path d="M0,0 L8,4 L0,8 z" fill="#8B5CF6" /></marker>
              </defs>
              {graph.edges.map((edge) => {
                ...
                const drag = edgeTargetDrag?.edgeKey === edge.key ? edgeTargetDrag : null;
                const targetPoint = drag ?? getEdgeTargetPoint(target, bounds.offsetX, bounds.offsetY);
                const path = drag
                  ? getEdgePathToPoint(source, targetPoint, bounds.offsetX, bounds.offsetY)
                  : getEdgePath(source, target, bounds.offsetX, bounds.offsetY);
                const sourceX = source.x + bounds.offsetX + FLOW_NODE_WIDTH;
                const sourceY = source.y + bounds.offsetY + FLOW_NODE_HEIGHT / 2;
                const labelX = (sourceX + targetPoint.x) / 2;
                const labelY = (sourceY + targetPoint.y) / 2 - 15;
                const stroke = isSelected ? '#8B5CF6' : '#64748B';

                return (
                  <g data-flow-edge-control data-flow-edge-key={edge.key} key={edge.key}>
                    <path d={path} fill="none" markerEnd={isSelected ? 'url(#business-arrow-selected)' : 'url(#business-arrow)'} stroke={stroke} strokeWidth={isSelected ? 3 : 2} />
                    <path
                      d={path}
                      fill="none"
                      onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedEdgeKey(edge.key); }}
                      pointerEvents="stroke"
                      stroke="transparent"
                      strokeWidth="18"
                      style={{ cursor: 'pointer' }}
                    />
                    <text fill={isSelected ? '#6D28D9' : '#475569'} fontSize="12" fontWeight="600" x={labelX} y={labelY}>{edge.label}</text>
                    {isSelected ? <circle
                      aria-label={String(t('dashboard.business.flow.moveEdgeTarget'))}
                      cx={targetPoint.x}
                      cy={targetPoint.y}
                      fill="#FFFFFF"
                      onLostPointerCapture={cancelEdgeTargetDrag}
                      onPointerCancel={cancelEdgeTargetDrag}
                      onPointerDown={(event) => { beginEdgeTargetDrag(event, edge); }}
                      onPointerMove={moveEdgeTarget}
                      onPointerUp={endEdgeTargetDrag}
                      pointerEvents="all"
                      r="8"
                      stroke="#8B5CF6"
                      strokeWidth="3"
            ...
            {graph.nodes.map((node) => {
              const isDropTarget = edgeTargetDrag?.hoverTargetKey === node.key;

              return <Card data-flow-node data-flow-node-key={node.key} key={node.key} onClick={() => { setSelectedEdgeKey(null); setSelectedKey(node.key); }} onPointerDown={(event) => { beginDrag(event, node); }} sx={{ border: '2px solid', borderColor: isDropTarget ? 'success.main' : selectedKey === node.key ? nodeColors[node.type] : 'divider', boxShadow: isDropTarget ? '0 0 0 5px rgba(34, 197, 94, .22)' : selectedKey === node.key ? `0 0 0 4px ${nodeColors[node.type]}24` : 2, cursor: 'grab', left: node.x + bounds.offsetX, position: 'absolute', top: node.y + bounds.offsetY, touchAction: 'none', transition: 'border-color 120ms ease, box-shadow 120ms ease', userSelect: 'none', width: FLOW_NODE_WIDTH, zIndex: 1, '&:active': { cursor: 'grabbing' } }}><CardContent sx={{ p: '14px !important' }}><Stack spacing={1}><Stack direction="row" spacing={0.5} sx={{ alignItems: 'center', justifyContent: 'space-between' }}><Chip label={t(`dashboard.business.flow.types.${node.type}`)} size="small" sx={{ bgcolor: `${nodeColors[node.type]}18`, color: nodeColors[node.type] }} /><Stack direction="row" spacing={0.5}>{node.type === 'instruction' && node.config.ai_message_enabled ? <Chip color="secondary" label={t('dashboard.business.flow.aiBadge')} size="small" /> : null}{node.config.visibility === 'internal' ? <Chip color="warning" label={t('dashboard.business.flow.internal')} size="small" /> : null}{node.config.finish_chat ? <Chip color="error" label={t('dashboard.business.flow.finishChat')} size="small" /> : null}{node.config.start ? <Chip color="success" label={t('dashboard.business.flow.start')} size="small" /> : null}</Stack></Stack><Typography noWrap variant="subtitle2">{node.title}</Typography><Typography color="text.secondary" sx={{ display: '-webkit-box', overflow: 'hidden', WebkitBoxOrient: 'vertical', WebkitLineClamp: 2 }} variant="caption">{node.type === 'instruction' ? (node.config.ai_message_enabled ? String(node.config.ai_instruction ?? '') : getInstructionMessages(node.config)[i18n.resolvedLanguage === 'en' ? 'en' : 'es']) : node.type === 'decision' && node.config.mode === 'knowledge_yes_no' ? getDecisionQuestions(node.config)[i18n.resolvedLanguage === 'en' ? 'en' : 'es'] : node.type === 'decision' ? String(node.config.mode ?? '') : String(node.config.action ?? '')}</Typography></Stack></CardContent></Card>;
            })}
          </Box>
        </Box>
        ...
        <Card sx={{ flex: '0 0 auto', width: { lg: 300, xs: '100%' } }}><CardContent><Stack spacing={2}><Typography variant="h6">{t('dashboard.business.flow.inspector')}</Typography>{selected ? <React.Fragment>
          ...
          {selected.type === 'instruction' ? <React.Fragment>
            <FormControlLabel control={<Switch checked={Boolean(selected.config.ai_message_enabled)} onChange={(_, checked) => { updateConfig(selected.key, { ai_message_enabled: checked }); }} />} label={t('dashboard.business.flow.aiMessage')} />
            {selected.config.ai_message_enabled ? <React.Fragment>
              <Alert severity="info">{t('dashboard.business.flow.aiMessageHelp')}</Alert>
              <TextField helperText={t('dashboard.business.flow.aiInstructionHelp')} label={t('dashboard.business.flow.aiInstruction')} multiline onChange={(event) => { updateConfig(selected.key, { ai_instruction: event.target.value }); }} rows={4} value={String(selected.config.ai_instruction ?? '')} />
              <FormControlLabel control={<Switch checked={selected.config.use_business_description !== false} onChange={(_, checked) => { updateConfig(selected.key, { use_business_description: checked }); }} />} label={t('dashboard.business.flow.useBusinessDescription')} />
              <FormControlLabel control={<Switch checked={selected.config.use_sources !== false} onChange={(_, checked) => { updateConfig(selected.key, { use_sources: checked }); }} />} label={t('dashboard.business.flow.useSources')} />
            </React.Fragment> : null}
            <Typography color="text.secondary" variant="caption">{t('dashboard.business.flow.messagesHelp')}</Typography>
            <TextField label={t('dashboard.business.flow.messageEs')} multiline onChange={(event) => { const messages = getInstructionMessages(selected.config); updateConfig(selected.key, { message: event.target.value, messages: { ...messages, es: event.target.value } }); }} rows={4} value={getInstructionMessages(selected.config).es} />
            <TextField label={t('dashboard.business.flow.messageEn')} multiline onChange={(event) => { const messages = getInstructionMessages(selected.config); updateConfig(selected.key, { messages: { ...messages, en: event.target.value } }); }} rows={4} value={getInstructionMessages(selected.config).en} />
            <FormControlLabel control={<Switch checked={Boolean(selected.config.finish_chat)} onChange={(_, checked) => { changeInstructionFinish(selected.key, checked); }} />} label={t('dashboard.business.flow.finishChat')} />
            {selected.config.finish_chat ? <Alert severity="info">{t('dashboard.business.flow.finishChatHelp')}</Alert> : <React.Fragment>
              <FormControlLabel control={<Switch checked={Boolean(selected.config.wait_for_input)} onChange={(_, checked) => { updateConfig(selected.key, { wait_for_input: checked }); }} />} label={t('dashboard.business.flow.waitForInput')} />
              {selected.config.wait_for_input ? <React.Fragment><TextField helperText={t('dashboard.business.flow.instructionRequiredFieldsHelp')} label={t('dashboard.business.flow.requiredFields')} onChange={(event) => { updateConfig(selected.key, { required_fields: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) }); }} value={((selected.config.required_fields as string[] | undefined) ?? []).join(', ')} /><TextField helperText={t('dashboard.business.flow.instructionOptionalFieldsHelp')} label={t('dashboard.business.flow.optionalFields')} onChange={(event) => { updateConfig(selected.key, { optional_fields: event.target.value.split(',').map((value) => value.trim()).filter(Boolean) }); }} value={((selected.config.optional_fields as string[] | undefined) ?? []).join(', ')} /></React.Fragment> : null}
            </React.Fragment>}
          </React.Fragment> : null}
          ...
          <Divider /><Typography variant="subtitle2">{t('dashboard.business.flow.connections')}</Typography>
          {selected.type === 'instruction' && selected.config.finish_chat ? <Alert severity="info">{t('dashboard.business.flow.finishChatConnectionHelp')}</Alert> : <React.Fragment>
            <FormControl><InputLabel>{t('dashboard.business.flow.target')}</InputLabel><Select label={t('dashboard.business.flow.target')} onChange={(event) => { setTargetKey(event.target.value); }} value={targetKey}>{graph.nodes.filter((node) => node.key !== selected.key).map((node) => <MenuItem key={node.key} value={node.key}>{node.title}</MenuItem>)}</Select></FormControl>
            {selected.type === 'decision' ? <FormControl><InputLabel>{t('dashboard.business.flow.branch')}</InputLabel><Select label={t('dashboard.business.flow.branch')} onChange={(event) => { setBranch(event.target.value); }} value={branch}>{((selected.config.branches as string[] | undefined) ?? []).map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</Select></FormControl> : null}
            <Button disabled={!targetKey || (selected.type === 'decision' && !branch)} onClick={addEdge} variant="outlined">{t('dashboard.business.flow.connect')}</Button>
          </React.Fragment>}
