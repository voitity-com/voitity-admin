...
export function BusinessFlowEditor({ graph, onChange }: { graph: BusinessFlowGraph; onChange: (graph: BusinessFlowGraph) => void }): React.JSX.Element {
  ...
  const addNode = (type: BusinessNodeType): void => {
    ...
    const config = type === 'instruction' ? { message: '', wait_for_input: true } : type === 'decision' ? { branches: ['yes', 'no'], mode: 'technology_interest' } : { action: 'extract_fields', required_fields: ['full_name', 'email', 'phone', 'whatsapp', 'project_summary'] };
  ...
  const addEdge = (): void => {
    if (!selected || !targetKey || selected.key === targetKey) return;
    const sourceHandle = selected.type === 'decision' ? branch || String((selected.config.branches as string[] | undefined)?.[0] ?? '') : null;
    const key = `${selected.key}-${targetKey}-${sourceHandle ?? 'default'}-${Date.now()}`;
    onChange({ ...graph, edges: [...graph.edges, { config: {}, key, label: sourceHandle, source: selected.key, source_handle: sourceHandle, target: targetKey }] });
